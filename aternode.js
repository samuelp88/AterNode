...
class Aternode extends EventEmitter{
    constructor() {
         ...
         */
        this.getServers = function (serverIndex = 1) {
            const options = {
                ...
                method: 'GET',
                headers: {
                    "Cookie": `${aternosSessionCookie};${tempCookie}`
                }
            };
            let data1 = '';
            ...
            return new Promise((resolve, reject) => {
                req.on('close', () => {
                    const serversRaw = new JSDOM(data1).window.document.getElementsByClassName('server-body');
                    const servers = [];
                    for (let i = 0; i < serversRaw.length; i++) {
                        servers.push(serversRaw.item(i).attributes.item(1).value);
        ...
        this.getAjaxToken = function () {
            ...
            return new Promise((resolve, reject) => {
                req.on('close', () => {
                    const token = new JSDOM(data1).window.document.scripts.item(3).text.trim().split(';')[1].split(' = ')[1].replace(/"/, '').replace('"', '');
                    AJAX_TOKEN = token;
                    resolve(token);
                });
        ...
        this.login = async function ({ user, password }) {
            ...
            let data2 = `user=${user}&password=${md5(password)}`;
            const options = {
                hostname: 'aternos.org',
                port: 443,
                path: buildURL('/panel/ajax/account/login.php', {}),
                method: 'POST',
                headers: {
                    "Cookie": `${tempCookie}`,
            ...
            let data1 = '';
            const req = https.request(options, (res) => {
                cookie = res.headers["set-cookie"][2];
                res.on('data', (chunk) => {
                    data1 += chunk;
                });
            ...
            return new Promise((resolve, reject) => {
                req.on('close', () => {
                    ...
                    if (data1.success && cookie) {
                        aternosSessionCookie = cookie.split(';')[0];
                        resolve({ sessionCookie: aternosSessionCookie, data: data1 });
                    }
                    else
        ...
        this.setSessionCookie = function (ATERNOS_SESSION) {
        ...
        this.start = async function () {
            ...
            const options = {
                hostname: 'aternos.org',
                port: 443,
                path: buildURL('/panel/ajax/start.php', { 'headstart': 0, 'access-credits': 0 }),
                method: 'GET',
                headers: {
        ...
        this.getStatus = function (callback) {
            ...
            const options = {
                hostname: 'aternos.org',
                port: 443,
                path: buildURL('/server', { 'headstart': 0, 'access-credits': 0 }),
                method: 'GET',
                headers: {
                    "Cookie": `${aternosSessionCookie};${aternosServerCookie};${tempCookie}`
                }
        ...
        this.autoConfirm = async function (callback) {
            ...
            if (status.queue) {
                ...
                if (queueTime > 10) {
                ...
                else if (queueTime <= 10 && queueTime > 5) {
                    setTimeout(() => {
                        this.autoConfirm(callback);
                    }, 60000);
                }
                    ...
                    const options = {
                        hostname: 'aternos.org',
                        port: 443,
                        path: buildURL('/panel/ajax/confirm.php', {}),
                        method: 'GET',
                        headers: {
                            "Cookie": `${aternosSessionCookie};${aternosServerCookie};${tempCookie}`
                        }
        ...
        var generateAjaxToken = function () {
            var key = randomString(16);
            var value = randomString(16);
            tempCookie = 'ATERNOS' + "_SEC_" + key + "=" + value;
            return key + ":" + value;
        };

        var buildURL = function (url, data) {
            data.SEC = generateAjaxToken();
